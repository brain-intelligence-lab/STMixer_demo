...
from models.STMixer import STMixerV3
...
parser.add_argument('-c', '--config', default='data/cifar100.yml', type=str, metavar='FILE',
...
parser.add_argument('-data-dir', metavar='DIR', default="./raw/",
...
def main(model, sml):
    ...
    if sml:
        ...
        alpha = 0.5
        lamb = 0.9
        train_loss_fn = loss_functions.SBDistillationLoss(criterion=train_loss_fn, temperature=temperature, alpha=alpha,
                                                          lamb=lamb).cuda()
        # train_loss_fn = loss_functions.CKALoss(criterion=train_loss_fn).cuda()
        # train_loss_fn = loss_functions.

    # train_loss_fn = loss_functions.TET_KL_loss(criterion=train_loss_fn).cuda()
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            output = model(input)
            # output, sim_loss = output
            loss = loss_fn(output, target)  # - 0.01 * sim_loss
...
if __name__ == '__main__':
    sml = True
    model = STMixerV3(img_size=32, downsample_times=2, in_channels=3, embd_dims=384,
                      T=1, mlp_ratio=4, depths=10, num_head=16, num_classes=100, sml=sml)
    main(model, sml)

    '''
    python -m torch.distributed.launch --nproc_per_node=2 train_timm.py --config data/cifar100.yml \
    --model stmixerv3 --seed 40
    '''